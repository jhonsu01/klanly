...
2. [Análisis funcional (qué replicamos de Skool)](#2-análisis-funcional-qué-replicamos-de-skool)
...
20. [Mapa de reciclaje rifas → SkoolClone](#20-mapa-de-reciclaje-rifas--skoolclone)
...
**Diferenciadores frente a Skool (que solo usa Stripe):**

| Necesidad | Skool original | SkoolClone (este plan) |
| --- | --- | --- |
...
## 2. Análisis funcional (qué replicamos de Skool)
...
Menú lateral observado en `skool.com/settings`:
...
| **Payouts** | "Payouts for community and affiliate earnings" — vía **Stripe Connect** ("Skool se asocia a Stripe"). |
...
```mermaid
graph TD
    A[Plataforma SkoolClone] --> B[ADMIN de plataforma]
...
> **Roles internos de comunidad** (heredados de Skool, dentro del alcance del productor): `Owner` → `Admin` → `Moderator` → `Member`. Son distintos del ADMIN de plataforma.
...
- **Afiliados:** comisión **40%** (configurable en `affiliate_referrals.commission_pct`) como en Skool. El link de referido lleva `?ref=<hash>`; al convertir, se crea el registro y se acumula al balance.
- **Payout automático:** vía **Stripe Connect** (onboarding "Skool se asocia a Stripe" → cuenta conectada del productor).
...
### 7.5 Facturación
Al pasar a `paid` se genera **factura** (nº correlativo, From/To, línea de membresía, método, total) — replicando el invoice observado en Skool. Se envía por email y queda en `Payment history`.
...
- **Theme** claro/oscuro (observado en Skool).
...
## 20. Mapa de reciclaje rifas → SkoolClone
